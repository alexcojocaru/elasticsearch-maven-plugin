...
{    
    ...
    @Test
    public void testUser2Exists() throws ElasticsearchClientException
    {
        ...
        // verify the user attributes
        Map userData = (Map)user.get("_source");
        Assert.assertEquals("otherAlexc", userData.get("name"));
        Assert.assertEquals(new Long(1388000499000L), userData.get("lastModified"));
    }
    ...
    @Test(expected = ElasticsearchClientException.class)
    public void testUser2DoesNotExistInOtherIndex() throws ElasticsearchClientException
    {
        client.get("/load_test_other_index/test_type/2", HashMap.class);
    }
